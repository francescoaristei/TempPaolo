...
struct osnoise_sample {
    ...
    int         lw_vmexit_count; /* lw vmexits interference (added by me) */
    int         hw_vmexit_count; /* hw vmexits interference (added by me) */
    u64         steal_time;
...
static void print_osnoise_headers(struct seq_file *s)
{
	...
	seq_puts(s, "   IN US  AVAILABLE  IN US     HW   HW_EX LW_EX STEAL    NMI    IRQ   SIRQ THREAD\n");
...
set_int_safe_time(struct osnoise_variables *osn_var, u64 *time)
{
	...
	do {
		...
		*time = time_get();
		...
		barrier();
    /*
     * the current thread has computed the int_counter above inside the 
     * do{}. If an interrupt with greater priority take its place in the 
     * processor, the next time local_read() is executed on the int_counter var
     * the value will be different from the one read above, because meanwhile
 u   * the local_t variable int_counter of the osn_var struct has been updated.
     * The update it's done by other methods (e.g trace_softirq_entry_callback()) which
     * are called each time a kind of interrupt (softirq, irqexit etc) happens
     * (the local_inc() method increment the variable int_counter of type local_t).
     * Therefore it will continue looping in the while. When the thread is not
     * interrupted in the whole while body, then the while condition is false
     * because int_counter == to the local_read() and the method end. (added by me)
    */
	} while (int_counter != local_read(&osn_var->int_counter));
...
void osnoise_trace_irq_exit(int id, const char *desc)
{
    ...
    /* here the duration of the IRQ is computed */
    ...
    /* here the duration is taken into account to compute the right
     * duration of less priority types of noised (softirqs, generic threads) */
	cond_move_softirq_delta_start(osn_var, duration);
	cond_move_thread_delta_start(osn_var, duration);
}
...
static void trace_softirq_exit_callback(void *data, unsigned int vec_nr)
{
    ...
    /* WHERE is defined trace_softirq_noise ? */
...
static int run_osnoise(void)
{
    ...
    /* added by me */
    u64 previous_steal, current_steal;
    u64 previous_vmexit, current_vmexit;
    u32 is_steal_time = 0;
    ...
    /* working on the host makes previous_steal and current_steal
     * always 0, which means is_vmexit always false 
    */
    if(static_key_false(&paravirt_steal_enabled)){
        cpu = get_cpu();
        previous_steal = paravirt_steal_clock(cpu);
        previous_vmexit = paravirt_vmexit_count(cpu);
        put_cpu();
    }
    #else
    previous_steal = 0;
	...
	do {
        ...
        if(static_key_false(&paravirt_steal_enabled)){
            cpu = get_cpu();
            current_steal = paravirt_steal_clock(cpu);
            current_vmexit = paravirt_vmexit_count(cpu);
            put_cpu();
        }
        #else
        current_steal = 0;
        current_vmexit_count = 0;
        #endif
        ...
        /* only if greater than threshold considered as noise (added by me) */
		if (noise >= threshold) {
			int interference = int_count - last_int_count;

            /* added by me */
            is_steal_time = current_steal - previous_steal;

            printk(KERN_DEBUG "The steal time is : %d", is_steal_time);

            /* just for testing purposes, to remove */
            if(is_steal_time < 0){
                osnoise_taint("negative steal time");
                goto out;
            }
            is_vmexit = current_vmexit - previous_vmexit;
            if(is_vmexit){


                /* if is a vmexit but the steal time has changed
                 * it means is a hw vm exit, else a lw one
                */
                if(is_steal_time){
                    hw_vmexit_count += is_vmexit;
                    steal_time += is_steal_time;
                }
                else
                    lw_vmexit_count += is_vmexit;
            }
            ...
            /*
             * From the set_int_safe_time is clear that if a noise has occurred (noise > threshold)
             * but the int_count - last_int_count operation is still 0, it means that the noise
             * has been caused by hardware (or vmexit) because otherwise the two variable would have been
             * different (added by me)
            */
        ...
        /* added by me */
        previous_vmexit = current_vmexit;
        previous_steal = current_steal;
