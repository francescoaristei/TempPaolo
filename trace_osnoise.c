...
static void print_osnoise_headers(struct seq_file *s)
{
	...
	seq_puts(s, "   IN US  AVAILABLE  IN US     HW   HW_EX   LW_EX   STEAL  QEMU  HW_NOISE   NMI    IRQ   SIRQ THREAD\n");
...
static int run_osnoise(void)
{
    ...
    /* added by me */
    u32 previous_steal, current_steal;
    u32 previous_lwexit, current_lwexit;
    u32 previous_hwexit, current_hwexit;
    u32 is_steal_time = 0;
    ...
    */
    if(static_key_false(&paravirt_steal_enabled)){
        ...
        previous_lwexit = paravirt_vmexit_count(cpu);
        previous_hwexit = paravirt_hwexit_count(cpu);
        put_cpu();
    }
    #else
    previous_steal = 0;
	...
	do {
        ...
        if(static_key_false(&paravirt_steal_enabled)){
            ...
            current_lwexit = paravirt_vmexit_count(cpu);
            current_hwexit = paravirt_hwexit_count(cpu);
            put_cpu();
        }
        #else
        current_steal = 0;
		...
		if (noise >= threshold) {
            ...
            /* necessary condition for a steal time update is a vmexit (lw or hw)
             * therefore it cannot happen a vmexit without a steal time update
             */
            is_steal_time = current_steal - previous_steal;
            if(is_steal_time){

                /* just one of the two gets incremented */
                lw_vmexit_count += current_lwexit - previous_lwexit;
                hw_vmexit_count += current_hwexit - previous_hwexit;

                steal_time += is_steal_time;
                qemu += noise - is_steal_time;
            }
            else{
                if(!interference){
                    hw_count++;
        ...
        /* added by me */
        previous_lwexit = current_lwexit;
        previous_hwexit = current_hwexit;
