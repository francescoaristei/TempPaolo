...
static void record_steal_time(struct kvm_vcpu *vcpu)
{
	...
	if (version & 1) /* equivalent to x % 2 */
    ...
    /* added by me, set the flag of the steal time struct */
    ...
    /*
     * next 3 lines added by me
     * Increase vmexit counter
     * initialization of st fields done at guest side
    */
     unsafe_get_user(count, &st->count, out);
     count += 1;
     unsafe_put_user(count, &st->count, out);

     /* debuggin through demsg it shows the count value increased */
     /* printk(KERN_DEBUG "The count field is: %lu", count); */
...
static int vcpu_enter_guest(struct kvm_vcpu *vcpu)
{
    ...
    /* also in case of lightweight vmexit update steal time and steal count added by me */
    record_steal_time(vcpu);
...
out:
    /* added by me */
    count_vmexit += 1;
    printk(KERN_DEBUG "The vmexits are now: %d", count_vmexit);
	return r;
}
